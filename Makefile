...
package: clean all
	cd .. && rm -f *.zip && zip -r processing_prepair.zip processing_prepair -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*Makefile*
	mv ../processing_prepair.zip .

upload: package
	plugin_uploader.py processing_prepair.zip
